...
var folder = 'apple/filters'
var isimage = ['brand','type']
var databasetable = 'apple_filters'
...
router.post('/:name/insert',verify.adminAuthenticationToken, upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        res.redirect(`/admin/dashboard/apple/${encodeURIComponent(name)}?message=${encodeURIComponent('Saved Successfully')}`);
    } catch (err) {
        console.error(err);
...
router.get('/:name/delete',verify.adminAuthenticationToken, async (req, res) => {
    ...
    try {
        ...
        res.redirect(`/admin/dashboard/apple/${encodeURIComponent(name)}/list`);
    } catch (err) {
        console.error(err);
        res.status(500).send('Internal Server Error');
    }
...
router.post('/:name/update', verify.adminAuthenticationToken, upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        res.redirect(`/admin/dashboard/apple/${encodeURIComponent(name)}/update?id=${encodeURIComponent(body.id)}&message=${encodeURIComponent('Updated Successfully')}`);
    } catch (err) {
        console.error(err);
        res.status(500).send('Internal Server Error');
    }
